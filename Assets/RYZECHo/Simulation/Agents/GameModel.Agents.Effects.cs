...
{
    private bool AddWorldEffect(WorldEffectKind kind, Vector2 position, float radius, float lifetime, Color color, string message)
    {
        return AddWorldEffect(kind, position, radius, lifetime, color, message, ActorType.Player);
    }

    private bool AddWorldEffect(WorldEffectKind kind, Vector2 position, float radius, float lifetime, Color color, string message, ActorType ownerType)
    {
    ...
    private bool TryPlaceTemporaryStructure(BuildToolKind tool, Vector2 target, float lifetime, string message)
    {
        return TryPlaceTemporaryStructure(tool, target, lifetime, message, ActorType.Player);
    }

    private bool TryPlaceTemporaryStructure(BuildToolKind tool, Vector2 target, float lifetime, string message, ActorType ownerType)
    {
    ...
    private bool IsLineBlockedByWorldEffect(Vector2 start, Vector2 end)
    {
    ...
    private static bool SegmentIntersectsCircle(Vector2 start, Vector2 end, Vector2 center, float radius)
    {
        ...
        var closest = new Vector2(start.X + (dx * t), start.Y + (dy * t));
