...
internal sealed partial class GameModel
{
    ...
    private bool IsInsideBombSite(Vector2 position, float padding = 0f)
    {
        return GetBombSites().Any(site => IsInsideBombSite(position, site.Id, padding));
    }

    private bool IsInsideBombSite(Vector2 position, ObjectiveSiteId siteId, float padding = 0f)
    {
    ...
    private void BuildMapGeometry()
    {
        ...
        var spawnPoints = new[] { new Point(1, 2), new Point(1, 4), new Point(1, 7), new Point(1, 9) };
        _spawnCells.AddRange(spawnPoints);
