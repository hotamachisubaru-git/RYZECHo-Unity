...
{
    private void DrawActors(Graphics graphics, Vector2 mousePosition)
    {
        if (_player.IsAlive)
        {
    ...
    private void DrawActor(Graphics graphics, Actor actor, Vector2 mousePosition)
    {
        ...
        var facing = new Vector2(center.X + (MathF.Cos(actor.FacingAngle) * 22f), center.Y + (MathF.Sin(actor.FacingAngle) * 22f));
        ...
        DrawStatusEffects(graphics, actor, new Vector2(center.X, center.Y - actor.Radius - 42f));
    ...
    private void DrawEnemy(Graphics graphics, Actor enemy)
    {
        ...
        var points = new[]
        {
            new Vector2(enemy.Position.X, enemy.Position.Y - enemy.Radius - 2f),
            new Vector2(enemy.Position.X + enemy.Radius + 2f, enemy.Position.Y),
            new Vector2(enemy.Position.X, enemy.Position.Y + enemy.Radius + 2f),
            new Vector2(enemy.Position.X - enemy.Radius - 2f, enemy.Position.Y),
        };
        ...
        DrawStatusEffects(graphics, enemy, new Vector2(enemy.Position.X, enemy.Position.Y - enemy.Radius - 26f));
    }
