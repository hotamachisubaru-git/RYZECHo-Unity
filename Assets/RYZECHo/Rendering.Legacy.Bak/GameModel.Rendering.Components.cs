...
internal sealed partial class GameModel
{
    ...
    private void DrawPortraitOrb(Graphics graphics, Vector2 center, float diameter, Color accent)
    {
    ...
    private void DrawStatusEffects(Graphics graphics, Actor actor, Vector2 origin)
    {
        var offsetY = 0f;
        if (IsActorOnHoneyTrap(actor))
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "鈍足", Color.FromArgb(255, 240, 188, 92));
            offsetY -= 16f;
        }

        if (IsActorInStaticField(actor))
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "妨害", Color.FromArgb(255, 136, 226, 140));
            offsetY -= 16f;
        }

        if (actor.Type == ActorType.Enemy && IsActorLockedDown(actor))
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "封鎖", Color.FromArgb(255, 230, 194, 88));
            offsetY -= 16f;
        }

        if (actor.Type == ActorType.Player && IsPlayerBreathingExposed())
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "呼吸漏", Color.FromArgb(255, 255, 132, 108));
            offsetY -= 16f;
        }

        if (actor.Type == ActorType.Player && _playerOverdriveTimer > 0f)
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "OD", Color.FromArgb(255, 255, 132, 92));
            offsetY -= 16f;
        }

        if (actor.Type == ActorType.Player && _playerGhostTimer > 0f)
        {
            DrawEffectTag(graphics, new Vector2(origin.X, origin.Y + offsetY), "幽歩", Color.FromArgb(255, 196, 132, 255));
        }
    }

    private void DrawEffectTag(Graphics graphics, Vector2 center, string text, Color accent)
    {
        var width = 44f;
