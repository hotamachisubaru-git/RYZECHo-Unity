...
internal sealed partial class GameModel
{
    private static ProgressProfile LoadProgressProfile()
    {
        try
        {
            if (File.Exists(ProgressProfilePath()))
            {
                ...
                var profile = JsonConvert.DeserializeObject<ProgressProfile>(json);
    ...
    {
        return Path.Combine(Application.persistentDataPath, "prototype-profile.json");
    }
    ...
    private void EmitCosmeticEliminationEffect(Vector2 position)
    {
